...
# https://github.com/roddhjav/apparmor.d/blob/main/apparmor.d/groups/ssh/sshd-session
''
  abi <abi/4.0>,

  include <tunables/global>

  @{exec_path} = ${pkgs.openssh}/libexec/sshd-session
  profile sshd-session @{exec_path} flags=(attach_disconnected) {
    ...
    @{bin}/userdbctl                   Px,
    ${pkgs.openssh}/libexec/sshd-auth     Px,
